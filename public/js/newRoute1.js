document.getElementById("newRouteForm").addEventListener("submit", function(event) {
    ...
    const iframeCodeError = document.getElementById("iframeCode-error");
    ...
    if (!iframeCodeInput.value.trim()) {
        showError(iframeCodeInput, iframeCodeError);
        formIsValid = false;
    } else {
        hideError(iframeCodeInput, iframeCodeError);
    }
